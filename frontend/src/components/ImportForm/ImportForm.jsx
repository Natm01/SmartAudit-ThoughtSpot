...
const ImportForm = ({ projects, onSubmit, loading }) => {
  ...
  const handleFileSelect = (file, type) => {
    ...
    if (type === 'libroDiario') {
      setFormData(prev => ({
        ...prev,
        libroDiarioFiles: [...prev.libroDiarioFiles, file]
      }));
    } else {
      setFormData(prev => ({
        ...prev,
        sumasSaldosFile: file
      }));
    }
  ...
  const handleMultipleFileSelect = (files, type) => {
    ...
    if (invalidFiles.length > 0) {
      setErrors(prev => ({
        ...prev,
        [type]: `Archivos no válidos: ${invalidFiles.join(', ')}`
      }));
    }
    ...
    if (validFiles.length > 0) {
      setFormData(prev => ({
        ...prev,
        libroDiarioFiles: [...prev.libroDiarioFiles, ...validFiles]
      }));
      setErrors(prev => ({
        ...prev,
        [type]: null
  ...
  const handleSubmit = (e) => {
    ...
    // Hacer obligatorio el archivo de Sumas y Saldos
    if (!formData.sumasSaldosFile) {
      newErrors.sumasSaldos = 'Debe seleccionar el archivo de Sumas y Saldos';
    }
  ...
  // Componente para el área de subida de archivos
  const FileDropZone = ({ type, label, files, isMultiple = false, error, required = false }) => {
          ...
          <div className="p-3">
            {!hasFiles ? (
              ...
              <div className="space-y-2 max-h-[80px] overflow-y-auto">
                  ...
                  files.map((file, index) => (
                    <div key={index} className="flex items-center justify-between bg-gray-50 p-2 rounded-lg group hover:bg-gray-100 transition-colors">
                      ...
                      <button
                        ...
                        className="ml-2 text-gray-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"
                      >
                ...
                {/* Indicador para agregar más archivos */}
                {isMultiple && (
                  <div className="text-center pt-2 border-t border-gray-200">
                    <p className="text-xs text-gray-600">
                      <span className="font-medium text-purple-600">Agregar más archivos</span> o arrastrar aquí
                    </p>
                  </div>
                )}
              </div>
